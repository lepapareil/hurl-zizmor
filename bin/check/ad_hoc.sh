...
while read -r action_file action_line action_name action_hash action_version ; do
	...
	echo "Version is missing as comment (#vx.y.z) for $action_name action at the end of line n°$action_line in ${color_red}$action_file${color_reset}"
        errors_count=$((errors_count+1))
    else
        echo "Version is present as comment (#vx.y.z) for $action_name action at the end of line n°$action_line in ${color_green}$action_file${color_reset}"
    fi
